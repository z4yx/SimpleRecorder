...
int camera_get_frame(struct picture_t *pic)
{
	...
	printf("DQBUF: index=%d, seq=%d, length=%d, timestamp=%d-%d\n", 
		cam_buf.index, cam_buf.sequence, cam_buf.length, (int)cam_buf.timestamp.tv_sec, (int)cam_buf.timestamp.tv_usec);
